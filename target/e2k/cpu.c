...
static void e2k_cpu_reset_hold(Object *obj, ResetType type)
{
    CPUState *cs = CPU(obj);
    ...
    if (ecc->parent_phasses.hold) {
        ecc->parent_phasses.hold(obj, type);
    }

    memset(env, 0, offsetof(CPUE2KState, end_reset_fields));

    if (tcg_enabled()) {
        e2k_cpu_init_fp_statuses(env);
    }
...
static struct TCGCPUOps e2k_tcg_ops = {
    ...
    .translate_code = e2k_tcg_translate_code,
...
static const Property e2k_cpu_properties[] = {
...
static void e2k_cpu_class_init(ObjectClass *oc, void *data)
{
    ...
    resettable_class_set_parent_phases(rc, NULL, e2k_cpu_reset_hold, NULL,
                                       &ecc->parent_phasses);
