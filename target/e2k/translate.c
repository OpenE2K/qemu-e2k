    ...
    MemOp memop, bool check)
{
    ...
    if (check && alop->ctx->mlock) {
        ...
        gen_helper_stmqp_mlock(alop->ctx->mlock, cpu_env, addr, s4.val, s2.val, sm, t0);
    } else {
        if (alop->ctx->version <= 5) {
            gen_helper_stmqp5(cpu_env, addr, s4.val, s2.val, sm);
        } else {
            gen_helper_stmqp6(cpu_env, addr, s4.val, s2.val, sm);
        }
    }
}
...
static void gen_aaurwd_aasti(Alop *alop, TCGv_i32 val, TCGv_i32 tag)
{
    ...
    gen_helper_aaurwd_aasti(cpu_env, aasti, val, tag);
}
...
static void gen_aaurwd_aaind(Alop *alop, TCGv_i32 val, TCGv_i32 tag)
{
    ...
    if (index) {
        ...
        gen_helper_aaurwd_aaind(cpu_env, aaind, val, tag);
    }
}
...
static void gen_aaurwd_aaincr(Alop *alop, TCGv_i32 val, TCGv_i32 tag)
{
    ...
    if (index) {
        ...
        gen_helper_aaurwd_aaincr(cpu_env, aaincr, val, tag);
    }
}
...
static void gen_aasti_incr(Alop *alop)
{
    ...
    if (rlp != 0) {
        ...
        tcg_gen_brcondi_i32(TCG_COND_EQ, t0, 0, l0);
        gen_helper_aasti_incr(cpu_env, aaind, aaincr, l);
        gen_set_label(l0);
    } else {
        gen_helper_aasti_incr(cpu_env, aaind, aaincr, l);
    }
...
static void gen_aad_ptr(DisasContext *ctx, TCGv ret, Alop *alop)
{
    ...
    gen_helper_aad_ptr(t0, cpu_env, tcg_constant_i32(alop->als.aad),
...
#error Not implemented
    ...
    case OP_QPACKHBSS:      gen_alopf1_qqq(alop, gen_qpackhbss); break;
    case OP_QPACKHBUS:      gen_alopf1_qqq(alop, gen_qpackhbus); break;
    case OP_QPACKWHSS:      gen_alopf1_qqq(alop, gen_qpackwhss); break;
    case OP_QPACKWHUS:      gen_alopf1_qqq(alop, gen_qpackwhus); break;
    case OP_VFSI:
    case OP_MOVTRS:
    case OP_MOVTRCS:
    case OP_MOVTRD:
...
{
    gen_helper_aau_load_program(cpu_env, cpu_ctprs[1]);
}
...
void e2k_tcg_initialize(void) {
    ...
    cpu_wreg = tcg_global_mem_new_ptr(cpu_env, offsetof(CPUE2KState, wreg), "wreg");
    cpu_wtag = tcg_global_mem_new_ptr(cpu_env, offsetof(CPUE2KState, wtag), "wtag");
    cpu_breg = tcg_global_mem_new_ptr(cpu_env, offsetof(CPUE2KState, breg), "breg");
    cpu_btag = tcg_global_mem_new_ptr(cpu_env, offsetof(CPUE2KState, btag), "btag");
    cpu_bcur = tcg_global_mem_new_i32(cpu_env, offsetof(CPUE2KState, bcur), "bcur");
}
